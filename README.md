...
stateDiagram-v2
    ...
    state Складская_Система {
        %% Разделяем систему на параллельные процессы: Склад и Доставка
        
        %% === ПОДПРОЦЕСС 1: ФУНКЦИОНИРОВАНИЕ СКЛАДА ===
        state Склад {
            [*] --> Рабочие_Дни
            Рабочие_Дни --> Пятница_Инвентаризация: Конец недели (Пятница)
            Пятница_Инвентаризация --> Выходные: Инвентаризация завершена
            Выходные --> Рабочие_Дни: Понедельник (Начало недели)
        ...
        %% === ПОДПРОЦЕСС 2: ЛОГИСТИКА И ПОСТАВКА ===
        state Логистика {
            [*] --> Ожидание_Запроса
