...
stateDiagram-v2
    ...
    state Складская_Система {
        ...
        state Склад {
            [*] --> Рабочие_Дни
            Рабочие_Дни --> Инвентаризация: Конец недели (Пятница)
            
            state if_order <<choice>>
            Инвентаризация --> if_order
            
            if_order --> Выходные: Если Остаток >= 600 (Без заказа)
            if_order --> Формирование_Заказа: Если Остаток < 600
            
            Формирование_Заказа --> Выходные: Сигнал "Сделать_Заказ" отправлен
            
            Выходные --> Рабочие_Дни: Понедельник

            Рабочие_Дни: <u>Обслуживание спроса</u><br>do/ Списание товара (40-63 шт/день)
            Инвентаризация: <u>Проверка остатка</u>
            Формирование_Заказа: <u>Оформление дозаказа</u><br>Объем = 1000 - Остаток
